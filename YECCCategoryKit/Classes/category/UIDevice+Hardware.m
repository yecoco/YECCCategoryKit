...
- (Hardware)hardware {
    ...
    if ([hardware isEqualToString:@"iPad5,1"])      return IPAD_MINI_4_RETINA_WIFI;
    if ([hardware isEqualToString:@"iPad5,2"])      return IPAD_MINI_4_RETINA_CELLULAR;
    if ([hardware isEqualToString:@"iPad5,3"])      return IPAD_AIR_2_WIFI_CDMA;
    if ([hardware isEqualToString:@"iPad5,4"])      return IPAD_AIR_2_WIFI_CELLULAR;
    if ([hardware isEqualToString:@"iPad5,5"])      return IPAD_PRO_WIFI;
    if ([hardware isEqualToString:@"iPad5,6"])      return IPAD_PRO_WIFI_CELLULAR;
...
- (NSString*)hardwareDescription {
    ...
    if ([hardware isEqualToString:@"iPad5,1"])      return @"iPad Mini 4 Retina (WiFi)";
    if ([hardware isEqualToString:@"iPad5,2"])      return @"iPad Mini 4 Retina (Cellular)";
    if ([hardware isEqualToString:@"iPad5,3"])      return @"iPad Air 2 (WiFi+CDMA)";
    if ([hardware isEqualToString:@"iPad5,4"])      return @"iPad Air 2 (WiFi+ Cellular)";
    if ([hardware isEqualToString:@"iPad5,5"])      return @"iPad Pro (WiFi)";
    if ([hardware isEqualToString:@"iPad5,6"])      return @"iPad Pro (WiFi+Cellular)";
...
- (NSString*)hardwareSimpleDescription
{
    ...
    if ([hardware isEqualToString:@"iPad5,1"])      return @"iPad Mini 4 Retina";
    if ([hardware isEqualToString:@"iPad5,2"])      return @"iPad Mini 4 Retina";
    if ([hardware isEqualToString:@"iPad5,3"])      return @"iPad Air 2";
    if ([hardware isEqualToString:@"iPad5,4"])      return @"iPad Air 2";
    if ([hardware isEqualToString:@"iPad5,5"])      return @"iPad Pro";
    if ([hardware isEqualToString:@"iPad5,6"])      return @"iPad Pro";
...
- (float)hardwareNumber:(Hardware)hardware {
    switch (hardware) {
        ...
        case IPAD_MINI_4_RETINA_WIFI:       return 5.1f;
        case IPAD_MINI_4_RETINA_CELLULAR:       return 5.2f;
        case IPAD_AIR_2_WIFI_CDMA:      return 5.3f;
        case IPAD_AIR_2_WIFI_CELLULAR:      return 5.4f;
        case IPAD_PRO_WIFI:         return 5.5f;
        case IPAD_PRO_WIFI_CELLULAR:        return 5.6f;
...
- (BOOL)isIphoneWith3point5inchDisplay
{
...
- (BOOL)isIphoneWith4point7inchDisplay
{
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
        double height = [[UIScreen mainScreen] bounds].size.height;
...
- (BOOL)isIphoneWith5point5inchDisplay
{
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
        double height = [[UIScreen mainScreen] bounds].size.height;
        if (fabs(height-763.0f) < DBL_EPSILON) {
            return YES;
        }
    }
    return NO;
...
- (BOOL)isIphoneWith5point8inchDisplay
{
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone) {
        double height = [[UIScreen mainScreen] bounds].size.height;
        if (fabs(height-812.0f) < DBL_EPSILON) {
            return YES;
        }
    }
    return NO;
